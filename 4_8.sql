-- ‘ормировать таблицу дл€ отчета будем последовательно.
WITH 
...
table_v2 AS -- добавл€ем к предыдущей таблице столбец rownum
(SELECT ROWNUM rn, department_id, salary, grouping_dep_id, grouping_salary
FROM table_v1),

table_v3 AS -- добавл€ем к предыдущей таблице столбец с именами соответствующих сотрудников и столбец с названи€ми отделов
...
ON t2.department_id = e.department_id AND NVL(t2.salary, 0) = NVL(e.salary, 0) -- NVL, иначе не получитс€ сравнить €чейки, где null значени€
AND t2.grouping_salary = 0 -- чтобы не внести изменени€ в строки промежуточных итогов
JOIN departments d
...
SELECT CASE
       ...
       CASE 
            WHEN grouping_salary = 1 THEN ('Ђ' || department_name || 'ї')
            ELSE NVL(salary, 0)
       END salary
FROM table_v3;
